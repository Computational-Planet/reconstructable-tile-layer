...
import { RotationOperator } from "rtl-finite-rotation";
import { QuadTreeTileProcessor, type NodeInfo } from "rtl-tile-plate-quadtree";
import type { CesiumTileProcessor, TileImageAsset } from "rtl-webgl-tile-processor";
...
export type {
  ...
  PaleoItem,
  PlateDomainFeature,
  PlateDomainQuadtreeGroup,
  PlateDomainQuadtreeRecord,
  PlateDomainSourceConfig,
  PlateQuadTreeGroup,
  PolygonQuadTreeRecord,
  ProcessedTileRecord,
  PrimitiveTransformMode,
  ReconstructableTileLayerOptions,
  ...
  ViewAwareTileLoadOptions,
  ViewAwareTileLoadResult,
  ViewAwareTileLoadSkipReason,
} from "./manager/types.js";
...
export class SimpleGeoReconstructManager {
  ...
  /** WebGL processing module used by this RTL instance. */
  get webglProcessor() {
    return this.processer;
  }

  /** Finite-rotation interpolator used for plate-specific transforms. */
  get finiteRotationInterpolator() {
    return this.rotationOperator;
  }

  /** Area features that participate in RTL tile reconstruction. */
  get plateDomainFeatures() {
    return this.paleoData;
  }

  /** Imported features, including diagnostic non-area records. */
  get allPlateDomainFeatures() {
    return this.allPaleoData;
  }

  /** Tile--plate quadtree records grouped by plate identifier. */
  get plateDomainQuadtrees() {
    return this.plates;
  }
  ...
  /** Returns retained RTL state and task-processing metrics. */
  getRuntimeStats() {
  ...
  /** Returns the most recent composite reconstruction-task report. */
  getLastReconstructionTaskReport(): TileGenerationReport | null {
    return this.getLastGenerationReport();
  }
  ...
  /** Prepares plate-domain indexes and finite rotations for this RTL instance. */
  async initialize() {
    return this.init();
  }
  ...
  /** Loads reconstruction tasks at an explicit source-tile level. */
  async loadSourceTilesAtLevel(viewer: Viewer, level: number) {
    return this.loadTilesOnLevel(viewer, level);
  }
  ...
  /** Refines the RTL from the current or supplied reconstructed-frame view. */
  async refineTilesForView(
    viewer: Viewer,
    options?: ViewFineTileLoadOptions,
  ): Promise<ViewFineTileLoadResult>;
  async refineTilesForView(
  ...
  async refineTilesForView(
    ...
    if (typeof levelOrOptions === "number") {
      return this.loadFineTilesInView(viewer, levelOrOptions, options);
    }
    return this.loadFineTilesInView(viewer, levelOrOptions);
  }
  ...
  /** Loads the adaptive root representation used for initial RTL display. */
  async loadRootTiles(viewer: Viewer) {
    return this.loadTilesAtRoot(viewer);
  }
  ...
  /** Applies an executable geological-age state in millions of years ago. */
  async setReconstructionAge(age: number) {
  ...
  /** Replaces the published Web tile service adapter used as RTL imagery input. */
  async setImageryProvider(viewer: Viewer, provider: ImageryProvider) {
    return this.setProvider(viewer, provider);
  }
