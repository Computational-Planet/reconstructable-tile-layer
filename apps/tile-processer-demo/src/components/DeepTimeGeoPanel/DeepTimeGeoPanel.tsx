...
import { GeographicTilingScheme, ImageryProvider, IonImageryProvider, UrlTemplateImageryProvider, WebMapTileServiceImageryProvider, WebMercatorTilingScheme } from "cesium";
...
const MapProvider: Record<string, ImageryProvider> = {
  ...
  "cesium-night":
    await IonImageryProvider.fromAssetId(3812, { accessToken: "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJqdGkiOiI4MTIzMzM3ZS1lNWEyLTRmNTAtYmI2Zi1hNjBlZTA3YTAyN2UiLCJpZCI6MTM1ODMzLCJpYXQiOjE2ODI1ODIzNjl9.04HzJGnDmmXKRbSzdhvE7epR9ny1xibwFRIZ1ipOM6Y" }),
  /* "bing-map": // 有bug，有待研究
...
function DeepTimeGeoPanel(props: DeepTimeGeoPanelProps) {
  ...
  const [mapProviderIndex, setMapProviderIndex] = useState<string>("arcgis-nature");
  ...
  useEffect(() => {
    if (viewerRef.current && simpleGeoReconstructManagerRef.current) {
      simpleGeoReconstructManagerRef.current.updateProvider(viewerRef.current, MapProvider[mapProviderIndex]);
    }
  }, [mapProviderIndex]);
  ...
  return (
    <div className={clsx(className)} style={{ ...userStyle }}>
      ...
      <div style={{ marginBottom: 10 }}>
        ...
        <input type="number" defaultValue={3} min={MapProvider[mapProviderIndex].minimumLevel} max={MapProvider[mapProviderIndex].maximumLevel} onChange={(e) => { setLevel(Number(e.target.value)) }} />
      </div>
      <div style={{ marginBottom: 10 }}>
        <Button
          onClick={async () => {
            if (viewerRef.current && simpleGeoReconstructManagerRef.current) {
              simpleGeoReconstructManagerRef.current.generateTilePrimitivesAtRoot(viewerRef.current);
            }
          }}
        >
