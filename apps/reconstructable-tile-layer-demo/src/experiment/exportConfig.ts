...
export type ExperimentExportContext = {
  ...
  initialized: boolean;
  level: number;
  manager: ReconstructableTileLayer | null;
  ...
  status: string;
  webglProcessor: WebGLTileProcessor | null;
  viewer: Viewer;
};
...
export function createExperimentExportInfo(
  ...
  return {
    ...
    stats: {
      geoTileStats: context.manager?.getRuntimeStats() ?? context.stats,
      tileProcesserStats: context.webglProcessor?.getRuntimeStats() ?? null,
    },
  };
}
