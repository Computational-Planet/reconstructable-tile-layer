...
type BenchmarkRuntime = {
  webglProcessor: WebGLTileProcessor;
  manager: ReconstructableTileLayer;
};
...
type InstallPerformanceBenchmarkOptions = {
  viewer: Viewer;
  getActiveManager: () => ReconstructableTileLayer | null;
...
async function runLoadCondition(
  ...
  if (viewAware) {
    await runtime.manager.refineTilesForView(viewer, level, {
    ...
    await runtime.manager.loadSourceTilesAtLevel(viewer, level);
  }
...
async function runAgeCondition(
  ...
  await runtime.manager.loadSourceTilesAtLevel(viewer, 4);
  await waitForRuntimeIdle(runtime, config);
  await waitForRenderedFrames(viewer, 1, config.idleTimeoutMs);
  ...
  for (const toAgeMa of config.ageSequenceMa) {
    ...
    const before = snapshot(runtime);
    const operationStart = now();
    await runtime.manager.setReconstructionAge(toAgeMa);
    const returnMs = now() - operationStart;
    const atReturn = snapshot(runtime);
...
function destroyRuntime(viewer: Viewer, runtime: BenchmarkRuntime) {
  ...
  runtime.webglProcessor.destroy();
...
function snapshot(runtime: BenchmarkRuntime): BenchmarkStatsSnapshot {
  return {
    capturedAtMs: now(),
    manager: runtime.manager.getRuntimeStats(),
    processor: runtime.webglProcessor.getRuntimeStats(),
    generationReport: runtime.manager.getLastReconstructionTaskReport(),
